...
func NewProposal(content Content, id uint64, submitTime time.Time) (Proposal, error) {
	p := Proposal{
		ProposalId: id,
...
func ProposalHandler(_ sdk.Context, c Content) error {
	...
	default:
		return sdkerrors.Wrapf(sdkerrors.ErrUnknownRequest, "unrecognized adminmodule proposal type: %s", c.ProposalType())
	}
}
